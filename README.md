...
This repository holds a **playable game**: the **specification package** (`docs/`), the finished **deterministic simulation core** (`sim-core/`) with the netcode milestones (M0–M4, plus most of M5's hardening) and the full content catalog implemented and test-gated, and a **Godot 4 front-end** (`godot/`) with a **full presentation layer** — event-stream rendering, interpolation, VFX/juice, real audio and procedural music — plus menus, lobby, challenges, localization, and an 8-arena net match. It is a game you *play*, not just a headless sim: **meta-progression and records** (personal bests, run history, achievement unlocks), **single-player difficulty** and a boss-kill **victory** screen, a hold-Tab **DPS meter**, and a **human-playable seat** in the real 8-arena netcode match against bots. (A prebuilt **Windows demo build** in `demos/` predates that presentation rebuild — build from source per [`TUTORIAL.md`](TUTORIAL.md) for the current game.) The spec came first on purpose — networking is expensive to retrofit — and the core was then built netcode-first against it; the presentation rebuild and fun/retention program are recorded in [`docs/09`](docs/09-rebuild-plan.md), and the (sign-off-pending) Roblox additional-platform track in [`docs/10`](docs/10-roblox-track.md). What remains open is the *live-Steam* half of the ship track (real SDR transport, real lobbies, Steam stats) — see [`docs/06`](docs/06-roadmap-risks-testing.md) and [`docs/09`](docs/09-rebuild-plan.md).

**See it run now:** for a quick look with no engine, the whole game loop also runs headlessly over the real sim —
```bash
...
| [`docs/10-roblox-track.md`](docs/10-roblox-track.md) | **Roblox platform track (plan for review).** The approved-in-concept *additional* platform: mapping the [`03`](docs/03-network-architecture.md) director + sharded arenas onto one Luau server authority, the `content.rs` → Luau exporter keeping a single catalog, presentation/monetization/compliance analysis, and the R0–R4 milestone plan with gates. No code yet. |
...
- **Implemented and test-gated:** milestones **M0–M4** are shipped (deterministic sim, shadow-sim/snapshots, match director + transport abstraction, 8-player reconnect + host migration, full content systems and the 96-weapon / 110-modifier catalog), and **M5 is partial** (chaos/soak/replay/clock-sync hardening done). See the per-milestone status annotations in [`docs/06`](docs/06-roadmap-risks-testing.md) — each points at the test that proves it. The workspace suite stands at **354 tests**, all green, M0 determinism gate PASS.
- **Presentation + retention layer executed:** the [`docs/09`](docs/09-rebuild-plan.md) presentation rebuild (event-stream rendering, interpolation, MultiMesh, full juice, real CC0 audio + procedural music) and a fun/retention wave (personal bests + run history, near-miss framing and achievement celebrations, a build-identity arsenal panel, a hold-Tab DPS meter, single-player difficulty + boss-kill victory, and a human-playable seat 0 in the net match) are shipped on the front-end. None of it altered the sim's competitive envelope (snapshot v24 is a pure added-field re-baseline; the MP/director path stays fixed to Normal).
...
- **Assumptions are labeled.** Anything reconstructed from the WC3 map (rather than verified against decompiled triggers) is marked *(inferred)*; where the shipped game deliberately diverges from the source (the original enemy roster and names), the docs say so explicitly — and the match arc itself now follows the source (15 minutes, 10-min step, swift end; restored by the fidelity program).
