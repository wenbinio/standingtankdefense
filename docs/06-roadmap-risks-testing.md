...
- **Remaining (content data, not new systems):** bulk import of the full 79‑weapon / 87‑upgrade catalog (the representative set already exercises every system), and a handful of **exotic source abilities** (summons, life/mana drain, rotating‑wave geometry) that need small new mechanics. Each is a tight, parallelizable task against the now‑complete data‑model seams.
