...
### M4 — Content depth & balance ◐ **IN PROGRESS** (modifier engine + Steam scaffold landed)
...
- **Done this pass:**
  - **The modifier / stacking engine** (`sim::modifiers`, `docs/05 §5.3`) — the genre's "everything stacks, additive within a source, multiplicative across sources" mechanic. Scoped modifiers (global / per-damage-type / multiplicative damage, attack speed, bounty %, flat income, max HP) aggregate deterministically and are consulted in damage resolution and weapon firing; modifiers are purchasable in the shop alongside weapons (`Offer` is now a weapon **or** a modifier). Serialized in snapshots, hashed into `state_checksum`, golden checksum re-baselined. Unit + wiring tests (e.g. +100% modifier deals exactly 2× damage; +100% attack speed halves cooldown).
  - **Steam results + adapter plan** (`net::results`, `net::steam`) — `MatchStats::from_placements` maps the director's authoritative placement to Steam stats/leaderboard rows (win = top half, Last Stand = sole survivor); `steam::steam_send_flags` documents-in-code the `Channel`→`ISteamNetworkingSockets` send-flag mapping so the Steam transport adapter is a drop-in behind the same `Inbound`/`Outbound` interface the in-process `Hub` uses. (The live Steam SDK binding can't build in this sandbox; it's a documented, tested scaffold.)
  - 93 tests workspace-wide, zero warnings, determinism gate green.
- **Remaining M4 content work (well-specified follow-on, parallelizable per system):** status effects (Poison/Frost/Fire/Spikes) on enemies + their deterministic ticking; the remaining attack types (Bounce/Barrage/Wave/Area) in combat; Mana Shield/armor/dodge defensive subsystems; enemy archetypes + per-round wave tables + the 10/15-min scaling curves; the **Samwise** boss (immune to normal weapons, `Clear`-only); and bulk import of the full 79-weapon / 87-upgrade catalog (mostly data entry now the systems exist). Each is a tight agent task against the existing data-model seams.
