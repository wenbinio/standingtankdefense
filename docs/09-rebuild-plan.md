...
> **Fun / retention wave executed (2026-07, after the fidelity program):** the game is now something you *play and come back to*, not just watch. Landed: a **retention core** — personal bests + run history persisted to `profile.cfg` `[records]`, a near-miss **"almost had it"** death framing, achievement-unlock **celebrations**, a **next-goal** progress readout, a death explanation + rotating **first-session tips**, and a confirm-gated **"Restart Run"** in the pause menu (the shop-header bug is fixed too — slots are rarity-weighted **mixed**, not a weapons/economy split); a **build-identity arsenal panel** (class-grouped, rarity-colored, live synergy readouts); a **playable MP seat** — you drive **seat 0** in the real 8-arena netcode match vs 7 bots (was spectator-only), your inputs flowing the authoritative client → director path, Esc-confirm to abandon; **single-player difficulty** (Easy / Normal / Hard at deploy via **[H]**; records & achievements Normal-gated); an **SP victory** framing — killing the boss shows a gold **"BOSS SLAIN — YOU SURVIVED THE ARC"** results variant, records a win, and the run continues; a **DPS meter** (hold-**Tab** ranked per-weapon damage-attribution overlay, with Spikes/Clear/Other pseudo-rows); and the **audio rebuild** landing real CC0 SFX (Kenney, see `CREDITS.md`) + procedural music stems with intensity mixing and wired menu sounds. Two behavior-neutral, checksummed authoritative fields back the new surfaces: an SP `difficulty` preset and a per-source **damage ledger** (`damage_by_weapon` + `poison_src`/`Hazard.source`/`Minion.source` attribution stamps) feeding the DPS meter. **Snapshot v24**; golden after the merged difficulty + damage-attribution passes: `0x0c0ba40d0da8aa23`; suite **354 tests**, determinism gate PASS. Still pending presentation work is unchanged (P2 art production, P4 music, P5 minor items). The **four post-source E-series systems** (`docs/02 §2.10–2.13` — score attack, run mutators, unlock-gated starting loadouts, and synergy set bonuses; the **E4** design track, its config surface sketched in `docs/05 §5.8`) remain **DRAFTED — AWAITING SIGN-OFF**, not implemented; and the **Roblox additional-platform track** (`docs/10`) is a **plan awaiting sign-off**, no code yet.
>
