...
1. **"Content is data, validated against [05] schemas and hashed into `content_hash`."** Reality: content is compiled Rust (`sim/src/content.rs`, 87 weapons / 94 modifiers) and the only content hash in use is the placeholder `DEMO_CONTENT_HASH = 0xC0DE_C0DE` (`godot/rust/src/lib.rs:251`), which defeats the version-drift gate the join handshake exists for. **Recommendation:** keep compiled-Rust content (it has served the balance/test workflow well) but amend docs/05 §5.7 to say so, and compute a real `content_hash` over the compiled catalog (a deterministic serialization of the tables). Externalized JSON/TOML remains a future option, not a prerequisite.
