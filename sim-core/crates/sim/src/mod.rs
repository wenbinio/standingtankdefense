...
fn timeline_entry(
    ...
    obj(vec![
        ...
        // Rescale ratio from the Steam clock onto this timeline. Consumers MUST
        // apply it to the per-entry `start_tick` gates in `waves` and
        // `boss_escort`, which ship at Steam scale (up to 45000) and are NOT
        // pre-rescaled: on the Roblox timeline 7 of 17 wave gates sit past the
        // 9000-tick boss and would simply never open, silently gutting the
        // roster. Every gate divides evenly by 6, so the rescale is exact.
        ("tick_scale_num", int(scale_num as i64)),
        ("tick_scale_den", int(scale_den as i64)),
...
// ===================== themes.json (`docs/12`) =====================
//
// A SEPARATE artifact, on purpose. `docs/12 §12.2`: themes live outside
// `content.json` so that adding, editing or swapping one does not move
// `content_hash` and does not invalidate the R3 trace corpus. Nothing below is
// reachable from `content_body()`; the only thing the two documents share is the
// catalog INDEX, which is identity in both.

/// One pack's entries for one table, as `{name, flavor, tip}` in index order.
/// Names are RESOLVED here (a pack that declines to rename falls back to the
/// catalog name), so a consumer never needs `content.json` to render a shop row.
fn theme_entries(
    n: usize,
    name: impl Fn(u16) -> &'static str,
    entry: impl Fn(u16) -> crate::themes::Entry,
) -> Value {
    Value::Arr(
        (0..n as u16)
            .map(|i| {
                let e = entry(i);
                obj(vec![("name", s(name(i))), ("flavor", s(e.flavor)), ("tip", s(e.tip))])
            })
            .collect(),
    )
}

fn theme_document_for(p: &'static crate::themes::ThemePack) -> Value {
    obj(vec![
        ("id", s(p.id)),
        ("label", s(p.label)),
        ("rarity_tiers", Value::Arr(p.rarity_tiers.iter().map(|t| s(t)).collect())),
        ("weapons", theme_entries(c::WEAPONS.len(), |i| p.weapon_name(i), |i| p.weapon(i))),
        ("modifiers", theme_entries(c::MODIFIERS.len(), |i| p.modifier_name(i), |i| p.modifier(i))),
    ])
}

/// The theme document. Deliberately carries NO `content_hash` and no catalog
/// numbers — only counts, so a port can assert its own load covers every index.
pub fn themes_document() -> Value {
    obj(vec![
        ("schema_version", int(1)),
        (
            "note",
            s("Display strings only (docs/12). A theme may change nothing the simulation can \
               observe: none of this reaches state_checksum or content_hash, and editing this \
               file cannot invalidate the trace corpus. Array index is identity, exactly as in \
               content.json."),
        ),
        ("default", s(crate::themes::DEFAULT_ID)),
        ("num_weapons", int(c::WEAPONS.len() as i64)),
        ("num_modifiers", int(c::MODIFIERS.len() as i64)),
        (
            "themes",
            Value::Arr(crate::themes::PACKS.iter().map(|p| theme_document_for(p)).collect()),
        ),
    ])
}

/// The bytes written to `roblox/src/shared/themes.json`.
pub fn themes_json() -> String {
    themes_document().pretty(3)
}

/// `themes.json` wrapped as a Luau ModuleScript returning the raw text — the
/// same trick `content_data_luau` plays, for the same reason (`src/shared/` runs
/// under a bare interpreter and cannot read files).
pub fn theme_data_luau() -> String {
    let json = themes_json();
    assert!(!json.contains("]==]"), "themes.json would break the Luau long-bracket literal");
    format!(
        "--!nonstrict\n         -- GENERATED by `cargo run -p sim --bin export-content` -- DO NOT EDIT.\n         -- Source of truth: sim-core/crates/sim/src/themes/\n         -- Mirror of roblox/src/shared/themes.json (`docs/12`). Presentation only:\n         -- nothing here is observable by the simulation or by content_hash.\n         return [==[\n{json}]==]\n"
    )
}
...
/// Repo-relative paths of the generated artifacts. `themes.json` /
/// `ThemeData.luau` are a SEPARATE pair from the catalog pair on purpose
/// (`docs/12 §12.2`) — a reskin regenerates them without touching `content.json`
/// or `content_hash`.
pub const OUTPUTS: [&str; 6] = [
    "roblox/src/shared/content.json",
    "roblox/src/shared/ContentData.luau",
    "roblox/src/shared/themes.json",
    "roblox/src/shared/ThemeData.luau",
    "roblox/test/vectors/fixed_vectors.json",
    "roblox/test/vectors/rng_vectors.json",
];

/// The artifacts' contents, in [`OUTPUTS`] order.
pub fn all_artifacts() -> [String; 6] {
    [
        content_json(),
        content_data_luau(),
        themes_json(),
...
/// `content.json` wrapped as a Luau ModuleScript returning the raw text.
///
/// The Luau side cannot read files: `src/shared/` may touch no Roblox global and
/// runs under a bare interpreter, yet every sim module reads the catalog at
/// module scope. So the JSON has to be *embedded* in a module. Generating that
/// module here — rather than pasting the JSON into a hand-written `Content.luau`
/// — makes the duplication a build product that regenerates with the catalog and
/// therefore cannot go stale. (It already did once: adding `tick_scale_*` moved
/// `content_hash` and left the hand-embedded copy behind.)
///
/// `]==]` cannot occur in the emitted JSON, so the long-bracket literal is safe.
pub fn content_data_luau() -> String {
    let json = content_json();
    assert!(!json.contains("]==]"), "content.json would break the Luau long-bracket literal");
    format!(
        "--!nonstrict\n         -- GENERATED by `cargo run -p sim --bin export-content` -- DO NOT EDIT.\n         -- Source of truth: sim-core/crates/sim/src/content.rs\n         -- Mirror of roblox/src/shared/content.json (`roblox/CONTRACTS.md` C1).\n         return [==[\n{json}]==]\n"
    )
}
...
mod tests {
    ...
    fn generation_is_byte_stable() {
        ...
        assert_eq!(themes_json(), themes_json());
        assert_eq!(fixed_vectors_json(), fixed_vectors_json());
        assert_eq!(rng_vectors_json(), rng_vectors_json());
    }

    /// `docs/12 §12.2`, the hard rule, as a test: **no theme string may appear in
    /// `content.json`**. If a pack's text ever leaks into the catalog document it
    /// has entered `content_hash` and invalidated the R3 corpus — which is exactly
    /// the failure this separation exists to make impossible.
    #[test]
    fn no_theme_string_reaches_the_catalog_document() {
        let catalog = content_json();
        for p in crate::themes::PACKS {
            for i in 0..c::WEAPONS.len() as u16 {
                let e = p.weapon(i);
                for t in [e.name, e.flavor, e.tip] {
                    assert!(
                        t.is_empty() || !catalog.contains(t),
                        "{} weapon[{i}] text leaked into content.json: {t:?}",
                        p.id
                    );
                }
            }
            for i in 0..c::MODIFIERS.len() as u16 {
                let e = p.modifier(i);
                for t in [e.name, e.flavor, e.tip] {
                    assert!(
                        t.is_empty() || !catalog.contains(t),
                        "{} modifier[{i}] text leaked into content.json: {t:?}",
                        p.id
                    );
                }
            }
    ...
    /// The theme artifact covers every index of every pack, with a non-empty
    /// resolved name / flavor / tip — the `docs/12 §12.3` coverage promise, checked
    /// on the emitted bytes rather than on the Rust tables.
    #[test]
    fn theme_artifact_covers_every_index() {
        let doc = Value::parse(&themes_json()).expect("themes.json parses");
        assert_eq!(doc.at("num_weapons").as_i64(), c::WEAPONS.len() as i64);
        assert_eq!(doc.at("num_modifiers").as_i64(), c::MODIFIERS.len() as i64);
        let themes = doc.at("themes").as_arr();
        assert_eq!(themes.len(), crate::themes::PACKS.len());
        assert!(themes.iter().any(|t| t.at("id").as_str() == doc.at("default").as_str()));
        for t in themes {
            let id = t.at("id").as_str();
            assert_eq!(t.at("rarity_tiers").as_arr().len(), 4, "{id} rarity tiers");
            for (key, n) in [("weapons", c::WEAPONS.len()), ("modifiers", c::MODIFIERS.len())] {
                let arr = t.at(key).as_arr();
                assert_eq!(arr.len(), n, "{id}.{key} count");
                for (i, e) in arr.iter().enumerate() {
                    assert!(!e.at("name").as_str().is_empty(), "{id}.{key}[{i}] name");
                    assert!(!e.at("flavor").as_str().is_empty(), "{id}.{key}[{i}] flavor");
                    assert!(!e.at("tip").as_str().is_empty(), "{id}.{key}[{i}] tip");
                }
            }
        }
    }
    ...
    fn no_floats_are_emitted() {
        ...
        for (path, a) in OUTPUTS.iter().zip(all_artifacts()) {
            // `ContentData.luau` is the same JSON inside a Luau long-bracket
            // literal; unwrap it so this proof covers it too rather than
            // skipping it — it is what the Luau side actually consumes.
            let json = match a.split_once("[==[\n") {
                Some((_, rest)) => rest
                    .strip_suffix("]==]\n")
                    .unwrap_or_else(|| panic!("{path}: unterminated long-bracket literal"))
                    .to_string(),
                None => a,
            };
            Value::parse(&json).unwrap_or_else(|e| panic!("{path}: {e}"));
        }
    }
}
