...
//! build (`docs/12 §12.4`).
//!
//! Text migrated out of `descriptions.rs`; `docs/12 §12.1` is explicit that this
//! work does not get bulldozed. `name` is left empty throughout: the wardens
//! display names are the catalog names, and the pack resolves them from
//! `content.rs` (see `themes` module docs).
//!
//! The only edits since the migration are **figures**. `docs/12 §12.5` is
//! absolute that a tip may not lie, and later balance passes (`docs/11 §11.8`'s
//! Max-HP / Mana-Shield retune among them) moved numbers these tips were quoting.
//! Every such figure now states what `content.rs` grants, and
//! `themes::tests::modifier_tips_quote_the_catalog_magnitudes` re-derives them
//! from the catalog so the next retune breaks the build instead of the tip. Voice,
//! rhythm and wording are otherwise untouched.
...
static MODIFIERS: [Entry; NUM_MODIFIERS] = [
    ...
    Entry { name: "", flavor: "Foundry overrun stamped EPIC and locked in the deep vault; the Wardens were told it would never leave Ashmark.", tip: "+40% damage, multiplicative on top." },  // 4
    ...
    Entry { name: "", flavor: "The quartermaster skims a margin off every coin the Greycoats turn in, and the ledger never forgets.", tip: "+20% to passive income." },  // 8
    Entry { name: "", flavor: "A heavier cut, written into the ledger by a quartermaster who stopped pretending the conscripts would be paid.", tip: "+50% to passive income." },  // 9
    ...
    Entry { name: "", flavor: "Warden masons doubled the wall on the wall they already had, the way a siege teaches you to never stop pouring stone.", tip: "Imbued Masonry: +4000 max HP, then +25%." },  // 11
    ...
    Entry { name: "", flavor: "A Hexwright spring fed into the hull, the borrowed skin topping itself off the way a well does.", tip: "Moonwell: +4000 mana shield, regenerating." },  // 13
    ...
    Entry { name: "", flavor: "The Relief never came, so the Wardens took their mending off the dying and bolted fresh plate while they were at it.", tip: "Mask of Death: +2500 max HP, +15 HP per kill." },  // 38
    ...
    Entry { name: "", flavor: "Hexwright greenwood grown through the hull, more wall and slower mending the closer to death the gunner runs.", tip: "Living Wood: +4000 max HP, 1.5% missing HP/sec." },  // 47
    ...
    Entry { name: "", flavor: "The Greycoats price the dead by the wound; every hundred you open is a coin in the ledger.", tip: "+1 gold per 60 damage dealt." },  // 53
    Entry { name: "", flavor: "A richer contract from the same bloody ledger, paying out every twenty points of ruin you deal.", tip: "+1 gold per 12 damage dealt." },  // 54
    ...
    Entry { name: "", flavor: "Conclave shieldwork that studies the siege as it stands, thickening a little more each round it endures.", tip: "+5000 shield, +800 more each round." },  // 56
    Entry { name: "", flavor: "A patched plate off an Ashmark casualty, just enough give before the seam parts.", tip: "Improved Masonry: +1500 max HP." },  // 57
    ...
    Entry { name: "", flavor: "Hull plating cut from a dead Ashmark engine, bolted over the old wounds.", tip: "+2500 max HP." },  // 62
    Entry { name: "", flavor: "A Foundry armor ration, thin but stamped and accounted for in the ledger.", tip: "+10 flat armor." },  // 63
    Entry { name: "", flavor: "Heavy Warden plate pulled off a tank that held this line three reliefs ago.", tip: "+4000 max HP." },  // 64
    Entry { name: "", flavor: "The Greycoats run the corpse-tally generous when the bodies pile this high.", tip: "+50% gold per kill." },  // 65
    Entry { name: "", flavor: "A Hexwright recharging-rite for the borrowed skin, drawing the pool back up a quarter faster than it bleeds.", tip: "Recharge: +4000 shield, +25% shield regen." },  // 66
    Entry { name: "", flavor: "A standing line in the quartermaster's ledger, paid out of Ashmark salvage each round.", tip: "+20 gold income per tick." },  // 67
    ...
    Entry { name: "", flavor: "A thin trickle on the Greycoat ledger, the kind command forgets to honor.", tip: "Magic Coin: +5 gold income per tick." },  // 72
    ...
    Entry { name: "", flavor: "An obscene coat of Hexwright life-work, more borrowed skin than any Warden was meant to wear, and it turns the blows it eats.", tip: "Energy Shield: +20000 mana shield." },  // 77
    ...
    Entry { name: "", flavor: "Warden salvage welded round Warden salvage, the wall thickening every round the line holds.", tip: "+5000 max HP, +800 more each round." },  // 80
    Entry { name: "", flavor: "A thin Hexwright coat of borrowed math, enough to spend before your own skin.", tip: "Mana Shield: +2000 mana shield." },  // 81
    ...
    Entry { name: "", flavor: "Foundry masons fit the old wall to a finer tolerance than the war deserves, and the thicker it stands the harder its guns hit.", tip: "Mastercrafted Masonry: +8000 max HP, +1% damage per 2000 max HP." },  // 83
    ...
    Entry { name: "", flavor: "A Hexwright sigil burned into the borrowed skin; while the shield holds, every shot carries a little of that stolen fire.", tip: "Arcane Mark: +8000 mana shield, +20% damage while it holds." },  // 85
    Entry { name: "", flavor: "The Conclave fed this thing on the dying and it learned to drink; each corpse on the line tops its borrowed skin back up.", tip: "Maw of Death: +4000 mana shield, +15 shield per enemy killed." },  // 86
    Entry { name: "", flavor: "Hexwright shielding wound so tight that when it finally fails it fails outward, a white concussion that lays the whole front rank flat.", tip: "Energy Pulse: +4000 mana shield; when it breaks, stun all enemies in 1200 for 0.5s." },  // 87
