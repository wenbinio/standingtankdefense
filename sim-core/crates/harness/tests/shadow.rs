...
fn no_false_positive_corrections() {
    ...
    assert!(runner.in_sync(), "client and shadow drifted with no input divergence");
...
fn injected_divergence_is_detected_and_corrected() {
    ...
    for kind in 0..kind_names.len() {
        let (mut runner, sc) = runner_at(inject_tick, digest_interval);
        assert_eq!(runner.corrections, 0);
        assert!(runner.in_sync(), "out of sync before injection (kind {kind})");
        ...
        assert!(
            ...
            kind, kind_names[kind], digest_interval
        );
        assert_eq!(runner.corrections, 1);
        assert_eq!(runner.last_correction_tick, Some(next_tick));
        assert_eq!(next_tick % digest_interval, 0, "correction must occur on a digest boundary");
        assert!(runner.in_sync(), "kind {} ({}): not in sync after correction", kind, kind_names[kind]);
        ...
        assert!(runner.in_sync(), "kind {} ({}): drifted after reconvergence", kind, kind_names[kind]);
    }
}
